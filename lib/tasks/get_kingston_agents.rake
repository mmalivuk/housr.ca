desc "Get kingston agents"
task :get_kingston_agents => :environment do
  ...
  page = operator.get('http://www.agentlocator.ca/tag/Kingston')
  n = 1
  ...
  loop do
    page.search(".AgentListSingle").each do |agent|
      ...
      Stub.create!(:first_name => first_name, :last_name => last_name, :company => company, :phone => office_phone, :city=> "Kingston")
    end
