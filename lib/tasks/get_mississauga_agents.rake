desc "Get Mississsauga agents"
task :get_mississauga_agents => :environment do
  ...
  page = operator.get('http://www.agentlocator.ca/tag/Mississauga')
  n = 1
  ...
  loop do
    page.search(".AgentListSingle").each do |agent|
      ...
      Stub.create!(:first_name => first_name, :last_name => last_name, :company => company, :phone => office_phone, :city=> "Mississauga")
    end
