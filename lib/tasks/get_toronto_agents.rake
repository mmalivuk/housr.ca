desc "Get Toronto agents"
task :get_toronto_agents => :environment do
  ...
  page = operator.get('http://www.agentlocator.ca/tag/Toronto')
  n = 1
  ...
  loop do
    page.search(".AgentListSingle").each do |agent|
      ...
      Stub.create!(:first_name => first_name, :last_name => last_name, :company => company, :phone => office_phone, :city=> "Toronto")
    end
